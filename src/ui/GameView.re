...
let currentLevel = (world) => World.currentLevel(world) |> Rationale.Option.default(List.hd(world.levels));
type inxPlace = { i: int, place: Types.place };
...
let viewport = (player: Types.player, area) => {
  ...
  let blocks = RList.repeat({Types.tile: WALL, state: Empty, tileEffect: NoEff, visible: false  }, size);
  
  let pt1 = area |> List.map(ys => blocks @ ys @ blocks);

  let row = pt1 |> List.hd 
                |> List.length 
                |> RList.repeat({Types.tile: WALL, state: Empty, tileEffect: NoEff, visible: false })
                |> RList.repeat(_, size);
...
let make = (~game: Types.game, ~movePlayer, ~takeStairs, ~useExit, _children) => {
  ...component,
  render: (_) => 
    <div>
