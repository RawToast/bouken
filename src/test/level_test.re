...
describe("Level.modify", () => {
  test("Can modify a tile", _ => {
    let modifiedLevel = blankWorld |> Level.modifyTile(0, 0, waterTile);

    expect(modifiedLevel.map |> List.hd |> List.hd |> (i => i.tile))
    |> toBe(WATER);
  })
});
...
describe("Area", () => {
  describe("findPlayer", () => {
    test("Can find the player", _ => {
      ...
      expect(Option.isSome(player)) |> toEqual(true);
    });
    test("Returns none when there is no player", _ => {
      let player = blankWorld |> (w => w.map |> Area.findPlayer);

      expect(Option.isSome(player)) |> toEqual(false);
    });
  });
  ...
  describe("findEnemy", () => {
    test("Can find an enemy", _ => {
      let enemy =
        ...
        |> Level.modifyTile(
             ...
             {
               tile: GROUND,
               state: Enemy({...initEnemy, id: "Enemy"}),
               tileEffect: NoEff,
               visible: false,
             },
           )
      ...
      expect(Option.isSome(enemy)) |> toEqual(true);
    });
    test("Returns none when there is no enemy", _ => {
      ...
      expect(Option.isSome(enemy)) |> toEqual(false);
    });
  });
  ...
  describe("movePlayer", () => {
    describe("When the Player is on a ground tile", () => {
      ...
      let playerArea =
        areaResult
        -> Option.getWithDefault({player: nfPlayer, area: [[]]});

      test("The move is successful", _ => {
        expect(Option.isSome(areaResult)) |> toEqual(true)
      });

      test("The player still exists", _ => {
        expect(playerArea.area |> Area.findPlayer |> Option.isSome)
        |> toBe(true)
      });
      ...
      test("The player is updated ", _ => {
        expect(
          playerArea.area
          ->Area.findPlayer
          ->Option.getWithDefault(nfPlayer)
          |> (p => p.stats.position),
        )
        |> toBeCloseTo(0.)
      });
